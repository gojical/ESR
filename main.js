// main.js

// Modules to control application life and create native browser window
const { app, BrowserWindow } = require("electron");
const { Menu, nativeImage, Tray } = require("electron");
const { Notification } = require("electron");
const sudo = require("sudo-prompt");

const fs = require("fs");
const os = require("os");
const scriptsStatus = {};

let tray;
const path = require("path");

const createWindow = () => {
  // Create the browser window.
  const mainWindow = new BrowserWindow({
    width: 800,
    height: 600,
    webPreferences: {
      preload: path.join(__dirname, "preload.js"),
    },
  });

  // and load the index.html of the app.
  mainWindow.loadFile("index.html");

  // Open the DevTools.
  // mainWindow.webContents.openDevTools()
};

let color = "green";
// let newF = [];

function updateTray(force = false) {
  const files = fs.readdirSync(`${os.homedir()}/.scripts2exec`);
  const scripts = files
    .filter((file) => file.split(".")[1] === "sh")
    .map((file) => {
      return file;
    });

  const lenKnownScripts = Object.keys(scriptsStatus).length;
  if (lenKnownScripts === 0) {
    scripts.forEach((script) => {
      console.log("load", script);
      scriptsStatus[script] = { status: "not-running" };
    });
  }

  //   console.log({ f: scripts.length, x: lenKnownScripts, v: scriptsStatus });

  if (force || scripts.length != lenKnownScripts) {
    console.log("============================= > render me");
    const running = [];
    const notRunning = [];

    for (const script in scriptsStatus) {
      const niceScriptName = script.replace(".sh", "");
      switch (scriptsStatus[script].status) {
        case "not-running":
          notRunning.push({
            label: `🟦   |   ${niceScriptName}`,
            submenu: [
              {
                label: "🟢  Start",
                type: "normal",
                click: () => {
                  const NOTIFICATION_TITLE = "Basic Notification";
                  const NOTIFICATION_BODY = `${script} must run!`;
                  scriptsStatus[script] = { status: "running" };

                  new Notification({
                    title: NOTIFICATION_TITLE,
                    body: NOTIFICATION_BODY,
                  }).show();
                  updateTray(true);
                  var options = {
                    name: "Script Runner",
                    // icns: "/Applications/Electron.app/Contents/Resources/Electron.icns", // (optional)
                  };
                  // sudo.
                  sudo.exec(
                    "echo hello",
                    options,
                    function (error, stdout, stderr) {
                      if (error) console.error(error);
                      console.log("stdout: " + stdout);
                    }
                  );
                  //   console.log(scriptsStatus);
                },
              },
              { label: "📖  Logs", type: "checkbox" },
            ],
          });
          break;
        case "running":
          running.push({
            label: `🟩   |   ${niceScriptName}`,
            submenu: [
              { label: "☠️  Stop", type: "normal" },
              { label: "📖  Logs", type: "checkbox" },
            ],
          });
          break;
      }
    }

    console.log(notRunning, running, scriptsStatus);

    const contextMenu = Menu.buildFromTemplate([
      ...notRunning,
      { type: "separator" },
      ...running,
      { type: "separator" },
      {
        label: "🟥   |   xVPN",
        submenu: [
          { label: "⏪  Restart", type: "normal" },
          { label: "📖  Logs", type: "checkbox" },
        ],
      },
    ]);
    tray.setContextMenu(contextMenu);
  }
}

function mainLoop() {
  setTimeout(async () => {
    if (color === "green") {
      tray.setImage(nativeImage.createFromPath("assets/black.png"));
      color = "black";
    } else {
      tray.setImage(nativeImage.createFromPath("assets/green.png"));
      color = "green";
    }
    mainLoop();
    updateTray();
  }, 500);
}

// This method will be called when Electron has finished
// initialization and is ready to create browser windows.
// Some APIs can only be used after this event occurs.
app.whenReady().then(() => {
  tray = new Tray(nativeImage.createEmpty());

  tray.setToolTip("This is my application.");
  tray.setTitle("???");
  tray.setContextMenu(
    Menu.buildFromTemplate([{ label: "⚠️   |   Run xVPN2", type: "normal" }])
  );
  //   createWindow()

  // const icon = nativeImage.createFromPath('data:image/png;base64,')

  mainLoop();
  //   updateFiles();
  //   tray.setImage(nativeImage.createFromPath("assets/black.png"))

  // console.log(newF)

  //   updateFiles();

  app.on("activate", () => {
    // On macOS it's common to re-create a window in the app when the
    // dock icon is clicked and there are no other windows open.
    if (BrowserWindow.getAllWindows().length === 0) createWindow();
  });
});

// Quit when all windows are closed, except on macOS. There, it's common
// for applications and their menu bar to stay active until the user quits
// explicitly with Cmd + Q.
app.on("window-all-closed", () => {
  if (process.platform !== "darwin") app.quit();
});

// In this file you can include the rest of your app's specific main process
// code. You can also put them in separate files and require them here.
